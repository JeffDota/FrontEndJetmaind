    ...
    this.agendaService.getAllAgendas().subscribe(
      (resp: any) => {
        ...
        this.agendaEntregaInformes.map((item: any) => {
          ...
          console.log('Agenda', item?.idDocente?.nombresApellidos);
          //- Responsable: ${item.docente[0].nombresApellidos}
          this.arrayCalendario.push({
            start: new Date(item.fechaInicio),
            end: new Date(item.fechaFin),
            title: `Estudiante: ${item.idEstudiantes[0].nombre} - Docente: ${item?.idDocente?.nombresApellidos}`,
            color: colors.blue,
            id: item._id,
          });
        })
