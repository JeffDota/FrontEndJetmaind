...
export class ProgramaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      console.log('Paso el tiempo de espera: 600ms');
  ...
  async recuperarDatosCiudad() {
    this.ciudadService.getAllCiudadesSinLimite().subscribe(async (resp: any) => {
      /* let nombreciudades: any = [];
      resp.data.forEach((element: any) => {
        nombreciudades.push({ item_id: element._id, nombre: element.nombre });
      }); */
      const listaCiudades = await resp.data.map((item: any) => {
        return {
          item_id: item._id,
          nombre: item.nombre,
        };
      });
      this.dropdownListCiudades = listaCiudades;
      if (this.programaSeleccionada) {
        this.programaSeleccionada.idCiudad.map(async (c: any) => {
          const findCiudadPersona = await this.dropdownListCiudades.find(
            (item: any) => {
  ...
  async recuperarDatosSucursales() {
    this.sucursalService.getAllSucursalesSinLimite().subscribe(async (resp: any) => {
      /* let nombreSucursal: any = [];
      resp.data.forEach((element: any) => {
        nombreSucursal.push({ item_id: element._id, nombre: element.nombre });
      }); */

      const listaSucursales = await resp.data.map((item: any) => {
        return {
          item_id: item._id,
          nombre: item.nombre,
        };
      ...
      this.dropdownListSucursales = listaSucursales;
      if (this.programaSeleccionada && listaSucursales) {
        this.programaSeleccionada.idSucursal.map(async (s: any) => {
          const findSucursalPersona = await this.dropdownListSucursales.find(
  ...
  async recuperarDatosMarcas() {
    this.marcaService.getAllMarcasSinLimite().subscribe(async (resp: any) => {
      ...
      if (this.programaSeleccionada && listaMarcas) {
        this.programaSeleccionada.idMarca.map(async (m: any) => {
  ...
  async recuperarDatosnombreProgramas() {
    this.nombreProgramaService.getAllnombrePrograma().subscribe(async (resp: any) => {
      /* let nombrePrograma: any = [];
      resp.data.forEach((element: any) => {
        nombrePrograma.push({ item_id: element._id, nombre: element.nombre });
      }); */

      const listaProgramas = await resp.data.map((item: any) => {
        return {
          item_id: item._id,
          nombre: item.nombre,
        };
      ...
      this.dropdownListNombreProgramas = listaProgramas;
      if (this.programaSeleccionada) {
        this.programaSeleccionada.idNombrePrograma.map(async (c: any) => {
          const findNombrePrograma = await this.dropdownListNombreProgramas.find(
            (item: any) => {
              return item.item_id === c;
            }
          );
