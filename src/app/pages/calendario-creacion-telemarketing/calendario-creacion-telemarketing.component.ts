...
@Component({
  selector: 'app-calendario-creacion-telemarketing',
  templateUrl: './calendario-creacion-telemarketing.component.html',
  styleUrls: ['./calendario-creacion-telemarketing.component.css']
})
export class CalendarioCreacionTelemarketingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.citasTelemarketingService.getAllSinLimiteCitasTelemarketing().subscribe(
      (resp: any) => {
  ...
  mostrarDatosModal(cita: any) {
    ...
    this.atributostablaCita = {
      'nombreAtributos': ['Estado', 'Telemercadista', 'Fecha cita', 'Fomra', 'Asesor asignado'
        , 'Programa', 'Representante', 'Telefono', 'Ciudad', 'Actividad Economica', 'Tarjeta de credito',
        'Tarjeta', 'Direccion Cita', 'Sucursal', 'Zoom', 'Fecha', 'Observaciones Asesor', 'Correo', 'Codigo Lead', 'Observaciones'],

      'idAtributos': [cita?.estado, cita?.addedUser?.nombresApellidos, fechacitaString, cita?.forma, cita?.asignado[0]?.nombresApellidos
        , cita?.idMarca?.map((resp: any) => resp.nombre), cita?.nombreApellidoRepresentante, cita?.telefono, cita?.ciudad, cita?.actividadEconomica, cita?.tarjeraCredito
        , cita?.tarjeta, cita?.terreno, cita?.idSucursal[0]?.nombre, cita?.zoom, cita?.fecha, cita?.observacionesAsesor, cita?.email, cita?.codigoLead, cita?.observaciones]
    };
