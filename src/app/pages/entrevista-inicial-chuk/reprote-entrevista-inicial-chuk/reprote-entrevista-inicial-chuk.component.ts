...
export class ReproteEntrevistaInicialChukComponent implements OnInit {
  ...
  createPDF(estudiante: any, representante: any, contrato: any, entrevista: any) {
    ...
    let preguntas: any = ['Apoyo Académico (R.I.N) ', 'Evaluaciones continuas', 'Talleres y conferencias virtuales con psicólogos de amplia trayectoria', 'Certificado', '¿Asiste a escuela o colegio bilingüe? '
      , '¿En casa alguien habla inglés?', '¿Ha estudiando inglés anteriormente?', '¿Tiene algún problema de aprendizaje?', '¿Por qué ha decidido estudiar inglés?'
      ,'observaciones','fechaInicio','fechaInicio','FechaIncorporacion','tiempoCapacitacion','examenIncial'];

    let estudiantesEntrevista: any = [];
    let docenteHorario: any = [];
    let preguntasEstudiante: any = [];
    entrevista.estudiantes1.map((resp: any, index: number) => {
