...
export class AsignarHorariosEstudianteService {
  ...
  buscarbyEstudiante(idEstudiante: any) {
    const headers = this.retornarHeader();
    return this.http.get(`${base_url}/asignarhorarioestudiante/buscar-grupo-estudiante/${idEstudiante}`, { headers: headers });
  }
  ...
  docentehorarioPorDia( dia:any, estado:boolean, sucursal:any){
    const headers = this.retornarHeader();
