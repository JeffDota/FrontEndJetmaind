...
export class CitasTelemarketingService {
  ...
  getAllSinLimiteCitasTelemarketing(){
    const headers = this.retornarHeader();
    return this.http.get(`${base_url}/citastelemarketing/allSinLimite`, { headers: headers });
  }
  ...
  cargarCitasTelemarketing3Dias (skip: number = 0){
    const headers = this.retornarHeader();
    return this.http.get(`${base_url}/citastelemarketing/allSinLimite3Dias?skip=${skip}`, { headers: headers })
    .pipe(
      tap( (resp:any) => {
