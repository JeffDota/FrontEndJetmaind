...
export class AsistenciaService {
  ...
  getAllByDocenteHorario(idDocente:any, idHorario:any){
    const headers = this.retornarHeader();
    return this.http.get(`${base_url}/asistencia/buscar/${idDocente}/${idHorario}`, { headers: headers });
  }
    ...
    .pipe(
      tap( (resp:any) => {
        ...
        const asistencias = resp.data.map((asistencia:any) => new Asistencia(asistencia._id, asistencia.idDocente, asistencia.idHorario, asistencia.temaTratado
            ,asistencia.fecha,asistencia.ausentes,asistencia.presentes,asistencia.prueba,asistencia.observaciones,asistencia.unidad,asistencia.leccion)
        );
