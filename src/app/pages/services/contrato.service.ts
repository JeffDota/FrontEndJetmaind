...
export class ContratoService {
  ...
  obtenerContratoByIdDesbloqueado(id:string){
    const headers = this.retornarHeader();
    return this.http.get(`${base_url}/contrato/desbloquedo/${id}`, { headers: headers })
  }
  ...
  updatecontratoSinNotificacion(id:string, contrato:Contrato){
    const headers = this.retornarHeader();
    return this.http.put(`${base_url}/contrato/update/${id}`, contrato, { headers: headers });
  }
  
  updatecontratoSinNotificacionDesbloqueado(id:string, contrato:Contrato){
    const headers = this.retornarHeader();
    return this.http.put(`${base_url}/contrato/update/desbloqueado/${id}`, contrato, { headers: headers });
  }
  ...
  enviarCorreoPeeacontrato( idContrato:any){
    const headers = this.retornarHeader();
    return this.http.get(`${base_url}/contrato/reenviar-peea/${idContrato}` ,{ headers: headers });
  }
    ...
    .pipe(
      tap( (resp:any) => {
        ...
        const contratos = resp.data.map((contrato:any) => new Contrato(contrato._id, contrato.fecha, 
          ...
          , contrato.marcasVendidas, contrato.pea, contrato.entrevistaInicial, contrato.campania, contrato.agendaEntrevista
          , contrato.becaFrances, contrato.codigoAnterior, contrato.modalidad, contrato.paginaLead, contrato.observacionesAcademicas
          , contrato.codigoAnterior));
    ...
    .pipe(
      tap( (resp:any) => {
        ...
        const contratos = resp.data.map((contrato:any) => new Contrato(contrato._id, contrato.fecha, 
          ...
          ,contrato.valorMatricula,contrato.valorTotal,contrato.numeroCuotas,contrato.formaPago,contrato.comentario,
          contrato.directorAsignado,contrato.fechaAprobacion, contrato.voucher, contrato.personaAprueba, contrato.codigo
          , contrato.marcasVendidas, contrato.pea, contrato.entrevistaInicial, contrato.becaFrances, contrato.codigoAnterior
          , contrato.modalidad, contrato.paginaLead, contrato.observacionesAcademicas, contrato.codigoAnterior));
  ...
  cargarContratosDirectores (skip: number = 0){
    const headers = this.retornarHeader();
    return this.http.get(`${base_url}/contrato/directores?skip=${skip}`, { headers: headers })
    .pipe(
      tap( (resp:any) => {
