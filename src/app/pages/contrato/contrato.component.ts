...
export class ContratoComponent implements OnInit {
  ...
  setDataFormRepresentante(event: any) {
    ...
    console.log(event);
    this.dataRepresentante.telefono = event.telefono.e164Number;
  ...
  recuperarDatosCiudad() {
    ...
    setTimeout(() => {
      //poner la ciudad del asesor
      /* const findPersona = this.dropdownListCiudades.find((resp: any) => {
        if (resp.item_id === this.personaService.persona.idCiudad[0]) {
          return resp;
        }
      });
      
      if (findPersona) {
        this.onItemSelect(findPersona);
        this.registerForm.get('idCiudad')?.setValue([findPersona]);
      } */
    }, 1000);
