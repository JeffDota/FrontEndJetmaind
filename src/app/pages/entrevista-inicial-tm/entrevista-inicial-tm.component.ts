...
export class EntrevistaInicialTmComponent implements OnInit {
  ...
  entrevistaInialTMModel = new EntrevistaInicialTM();
  ...
  constructor(
    ...
    private entrevistaInicialTMService: EntrevistaInicialTmService,
  ...
  crear() {
    ...
    this.entrevistaInialTMModel = this.registerForm.value;
    this.entrevistaInialTMModel.idContrato = this.idContrato;
    if (this.registerForm.invalid) {
      ...
      this.entrevistaInicialTMService.crearEntrevista(this.entrevistaInialTMModel).subscribe((resp) => {

        //actualizar datos del contrato
        this.contratoService.obtenerContratoById(this.entrevistaInialTMModel.idContrato).subscribe((resp: any) => {
          let datos = resp.data;
          datos.estadoPrograma = 'Entrevista realizada';

          const marcasRepetidas = datos.entrevistaInicial.find((element: any) => {
            if (element.marca == 'Tomatis') {
              return true;
            } else {
              return false;
            }
          ...
          setTimeout(() => {
            this.contratoService.updatecontratoSinNotificacion(this.entrevistaInialTMModel.idContrato, datos).subscribe((resp: any) => {
              console.log(resp);
            });
          }, 100);
        ...
        this.router.navigateByUrl('/listaentrevistainicialtm');
      }, (err: any) => {
