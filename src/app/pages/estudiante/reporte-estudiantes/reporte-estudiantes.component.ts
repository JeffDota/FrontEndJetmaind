...
export class ReporteEstudiantesComponent implements OnInit {
  ...
  public dropdownListEstados: any = [];
  ...
  recuperarDatosEstados() {

    this.dropdownListEstados = [
      { item_id: 'Activo', nombre: 'Activo' },
      { item_id: 'Espera', nombre: 'Espera' },
      { item_id: 'Congelado', nombre: 'Congelado' },
      { item_id: 'Incorporado', nombre: 'Incorporado' },
      { item_id: 'Retirado', nombre: 'Retirado' },
      { item_id: 'Rechazado', nombre: 'Rechazado' },
    ];
  ...
  obtenerDatos(datos: any) {
    this.programaService.allByCiudadMarcaSucursalNombreprograma(datos).subscribe((resp: any) => {
      console.log('REPORTE ESTUDIANTES: ',resp);
  ...
  buscar() {
    ...
    datos.idEstado = this.estado.map((resp: any) => {
      return resp.item_id;
    });
  ...
  /** Deselccionar item */
  findByItemIdIndexEstado(id: any) {
    return this.estado.findIndex((resp: any) => {
      return resp.item_id === id;
    })
  }
  onDeSelectEstado(item: any) {
    /** Borrar elemento del array  */
    const index = this.findByItemIdIndexEstado(item.item_id);
    const newArray = (index > -1) ? [
      ...this.estado.slice(0, index),
      ...this.estado.slice(index + 1)
    ] : this.estado;
    this.estado = newArray;
    console.log(this.estado);
  }
  /** Deselccionar todos los items */
