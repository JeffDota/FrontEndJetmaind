...
export class EstudianteComponent implements OnInit, OnChanges {
  ...
  async cargarEstudiantebyId(id: string) {
    ...
    this.estudianteService.obtenerEstudianteById(id).subscribe((resp: any) => {
      ...
      //se realiza para Optimizar editar
      this.representanteService.obtenerRepresentanteById(this.estudianteSeleccionada.idRepresentante[0]).subscribe((resp: any) => {
        let nombrerepresentantes: any = [{
          item_id: resp.data._id,
          nombre: resp.data.nombresApellidos,
        }];
        this.dropdownListRepresentantes = nombrerepresentantes;
        this.onItemSelectRepresentante( this.dropdownListRepresentantes[0]);
        this.registerForm.get('idRepresentante')?.setValue(this.representante);
      });
  ...
  crearEstudiante() {
    if (this.estudianteSeleccionada) {
      ...
      this.EstudianteModel.telefono = this.registerForm.value.telefono.e164Number;
      if (this.registerForm.invalid) {
        ...
        console.log('PRUEBA EDICION: ', this.EstudianteModel);
